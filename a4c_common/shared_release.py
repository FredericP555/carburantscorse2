...
def _request_headers(url: str) -> dict[str, str]:
    headers = {"User-Agent": "A4C-carburantscorse2/2.0", "Accept": "application/vnd.github+json"}
...
def _select_shared_release(releases: Iterable[dict], *, tag_prefix: str = DEFAULT_TAG_PREFIX) -> dict:
    candidates = [r for r in releases if not r.get("draft") and str(r.get("tag_name", "")).startswith(tag_prefix)]
    if not candidates:
        raise RuntimeError(f"No published GitHub Release found with prefix {tag_prefix!r}")
    return max(candidates, key=lambda r: str(r.get("published_at") or r.get("created_at") or ""))
...
def _decode_snapshot(data_bytes: bytes, metadata: dict, years: Iterable[int]) -> list[dict]:
    ...
    if not requested_years.issubset(available_years):
        raise RuntimeError(f"Shared snapshot years {sorted(available_years)} do not cover {sorted(requested_years)}")
        ...
        with io.TextIOWrapper(gz, encoding="utf-8", newline="") as text:
            for raw in csv.DictReader(text):
                ...
                rows.append({
                    ...
                    "pop": raw.get("pop", ""), "is_motorway": _as_bool(raw.get("is_motorway")),
                    "latitude": raw.get("latitude", ""), "longitude": raw.get("longitude", ""),
                    "fuel_id": raw.get("fuel_id", ""), "fuel": raw.get("fuel", ""),
                    "timestamp": timestamp, "date": day,
...
def load_shared_observations(years: Iterable[int], *, repository: str = DEFAULT_REPOSITORY, tag_prefix: str = DEFAULT_TAG_PREFIX, release_tag: str | None = None) -> tuple[list[dict], dict]:
    years = sorted({int(year) for year in years})
    release, metadata = _release_and_metadata(repository=repository, tag_prefix=tag_prefix, release_tag=release_tag)
    data_bytes = _request_bytes(_asset_url(release, DATA_ASSET), timeout=240)
    rows = _decode_snapshot(data_bytes, metadata, years)
    source = {
        "kind": "c1-github-release", "repository": repository,
        "release_tag": release.get("tag_name"), "release_published_at": release.get("published_at"),
        "schema": metadata.get("schema"), "sha256": metadata.get("sha256"),
        "shared_source_max_date": metadata.get("max_date"), "shared_rows": metadata.get("rows"),
        "bouclier": metadata.get("bouclier"), "rotterdam": metadata.get("rotterdam"),
        "corse_station_brands": metadata.get("corse_station_brands"),
    }
    return rows, source


def download_shared_rotterdam_assets(output_dir: str | Path = "outputs/ufip", *, repository: str = DEFAULT_REPOSITORY, tag_prefix: str = DEFAULT_TAG_PREFIX, release_tag: str | None = None, registry_output: str | Path = "outputs/c1/corse_station_brands.json", tag_output: str | Path = "outputs/c1/shared_release_tag.txt") -> dict:
    release, metadata = _release_and_metadata(repository=repository, tag_prefix=tag_prefix, release_tag=release_tag)
    ...
    if hashlib.sha256(observed_bytes).hexdigest() != str(rotterdam.get("observed_sha256") or ""):
        raise RuntimeError("Shared Rotterdam observed asset SHA-256 mismatch or missing hash")
    if hashlib.sha256(daily_bytes).hexdigest() != str(rotterdam.get("daily_sha256") or ""):
    ...
    if hashlib.sha256(brands_bytes).hexdigest() != str(brands_meta.get("sha256") or ""):
    ...
    if brands_payload.get("schema") != CORSE_BRANDS_SCHEMA or not isinstance(brands_payload.get("stations"), dict) or not brands_payload["stations"]:
        raise RuntimeError("Invalid shared Corsica brand registry")
    out = Path(output_dir); out.mkdir(parents=True, exist_ok=True)
    observed_path = out / ROTTERDAM_OBSERVED_ASSET; daily_path = out / ROTTERDAM_DAILY_ASSET
    meta_path = out / "c1_shared_meta.json"
    observed_path.write_bytes(observed_bytes); daily_path.write_bytes(daily_bytes)
    meta_path.write_text(json.dumps(metadata, ensure_ascii=False, indent=2), encoding="utf-8")
    registry_path = Path(registry_output); registry_path.parent.mkdir(parents=True, exist_ok=True); registry_path.write_bytes(brands_bytes)
    tag_path = Path(tag_output); tag_path.parent.mkdir(parents=True, exist_ok=True); tag_path.write_text(selected_tag + "\n", encoding="utf-8")
    return {"repository": repository, "release_tag": selected_tag, "release_published_at": release.get("published_at"), "observed_file": str(observed_path), "daily_file": str(daily_path), "metadata_file": str(meta_path), "registry_file": str(registry_path), "tag_file": str(tag_path), "corsica_calibration": rotterdam.get("corsica_calibration")}
